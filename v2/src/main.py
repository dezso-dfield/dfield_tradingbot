...
from .charts import save_chart, save_annotated_top_chart
...
def main():
    ...
    pairs = []
    df_map = {}  # NEW: keep the latest DF per symbol for annotated charts
    ...
    for coin_id, symbol in universe:
        ...
        try:
            ...
            # Keep a copy for annotated charts later
            df_map[symbol] = df.copy()
            ...
            if args.use_binance:
                ...
                ind.live_price = float(ind.latest_close)  # proxy for daily mode
    ...
    # Annotated images for Top 10
    ind_map = {ind.symbol: ind for (ind, _d) in pairs}
    topN = ranked[:10]
    for r in topN:
        ind = ind_map[r.symbol]
        dec = r.decision
        df = df_map.get(r.symbol)
        if df is None:
            continue
        try:
            save_annotated_top_chart(
                df=df.tail(240),
                symbol=r.symbol,
                decision=dec,
                ind=ind,
                out_dir="outputs/top10_charts",
                mas=(20, 50, 200),
                lookback=200
            )
        except Exception as e:
            print(f"[yellow]Annotated chart failed for {r.symbol}, falling back to simple chart:[/] {e}")
            try:
                save_chart(df.tail(240), r.symbol, out_dir="outputs/top10_charts", mas=(20,50,200))
            except Exception as e2:
                print(f"[yellow]Simple chart also failed for {r.symbol}:[/] {e2}")
    ...
    for x in ranked[:10]:
        d = x.decision
        ind = ind_map.get(x.symbol)
        px = getattr(ind, "live_price", None) or getattr(ind, "latest_close", None)
        price_str = f"{px:.6g}" if px is not None else "n/a"
        print(f"{x.symbol:<6} px={price_str:<10} score={x.score:6.1f}  {d.action.upper():5} {d.bias:<7} "
            f"conf={d.conviction:.2f}  inv={d.invalidation}  tgts={d.targets}")
